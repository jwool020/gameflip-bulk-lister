...
const CONST = {
  ...
  LOG_NAME_FROM_LEVEL: Object.keys(Bunyan.nameFromLevel).reduce(function (
    map,
    key
  ) {
...
const Util = {
   ...
   */
  emptyArray: (x) =>
    typeof x === "undefined" || (Array.isArray(x) && x.length == 0),
  ...
  sleep: (delayInSeconds) =>
    new Promise((callback) => setTimeout(callback, delayInSeconds * 1000)),
};
...
class GfApi {
  ...
  constructor(apiKey, secret, options = {}) {
    ...
    this.baseUrl =
      split.length == 1 ? CONST.BASE_URL.production : CONST.BASE_URL[split[0]];
    ...
    this.log = Bunyan.createLogger({
      ...
      stream: {
        ...
        write: function (json) {
          ...
          console.log(
            "%s %s",
            CONST.LOG_NAME_FROM_LEVEL[logEntry.level],
            logEntry.msg
          );
    ...
    this.rateLimiter = new Limiter.RateLimiter({
      tokensPerInterval: 3,
      interval: "minute",
    });
  }
  ...
  wallet_get(owner, opt) {
    ...
    if (!options.year_month && typeof options.balance_only === "undefined")
      options.balance_only = true;
  ...
  async upload_photo(listing_id, file_or_url, display_order) {
    let photo =
      file_or_url.indexOf("https://") >= 0
        ? await this._readPhotoFromUrl(file_or_url)
        : await this._readPhotoFromFile(file_or_url);
    ...
    if (photo_data.length > CONST.IMAGE_BYTE_SIZE) {
      throw new Error(
        `ERROR Image file size over limit: ${photo_data.length} > ${CONST.IMAGE_BYTE_SIZE}`
      );
    }
  ...
  steam_inventory_get(profileid, appId, query = {}) {
    ...
    const contextId =
      CONST.STEAM.CONTEXT_ID[appId] || CONST.STEAM.CONTEXT_ID.DEFAULT;
    const url = `http://steamcommunity.com/inventory/${profileid}/${appId}/${contextId}${Util.queryString(
      query
    )}`;
    this._entry(`GET '${url}'`);
  ...
  _getList(uri, query) {
    ...
    const url =
      query.nextPage || `${this.baseUrl}/${uri}${Util.queryString(query)}`;
    this._entry(`GET ${url}`);
  ...
  async _apiRequest(request, query = null) {
    try {
      ...
      if (apiData.status == "SUCCESS") {
        ...
        const statusMessage = Util.options(
          error.message,
          response.statusMessage
        );

        this.log.trace(
          `FAIL: statusCode=${statusCode} statusMessage=${statusMessage}`
        );
  ...
  async _steamRequest(request, query = null) {
    try {
      ...
      if (apiData.success) {
        ...
        if (
          Util.isNull(apiData.more_items) &&
          Util.emptyArray(apiData.assets)
        ) {
          return null;
        }
        ...
        const response = result.response;
        this.log.trace(
          `FAIL: statusCode=${response.statusCode} statusMessage=${response.statusMessage}`
        );
  ...
  async _readPhotoFromUrl(url) {
    ...
    if (result.data) {
      ...
      const { default: FileType } = await import("file-type"); // <-- ADD THIS LINE
